...
| 2026-09-07b | **Single-buffer rung — VERDICT REJECTED (null), 1-barrier anti-WAR refill — VERDICT LANDED (+5-9%)** (`spirv_coopmat_stages`, default 2) | (1) Single-buffer (halve smem → 8 WGs/SM = 16 warps): **IDENTICAL perf** (5.13-5.24 vs 5.16-5.24ms) — the smem occupancy is NOT binding at S=2 (registers cap the WGs); knob added but defaults to 2. (2) **Anti-WAR refill**: the loop refill now writes the OTHER stage (1-s, panel kt+1) instead of the stage the CM loads just read (s, panel kt+2). The fill can no longer race the in-flight CM loads of stage s → the workgroup barrier between the mma and the fill is DROPPED (double-buffer only). Result: the fill's DRAM loads issue right after the mma issues (no completion-wait), overlapping the tensor work — a register-free partial D2. In-process A/B vs the 2-barrier form (4096³, 5 rounds, both 4.436e-03 OK): clean rounds **0.913/0.954/0.955/0.949** → **~5.04-5.11ms vs 5.30-5.60ms**. SPV barrier count 4→3 (2 prologue + 1 loop). | 2-barrier S=2 5.30-5.60ms | **The 2-barrier double-buffer serialized the fill behind the mma completion; writing the spare stage removes the WAR and the barrier. Next lever: hide the fill stores behind the mma too (the register-free D2 is only partial).** |
| 2026-09-07c | **D2 prefetch LANDED at S=2 (+8-11%)** (`spirv_coopmat_fill_prefetch`, default 0 → 1) | the D2 register-prefetch (fill DRAM loads issue at the loop top, smem stores after the barrier) was REJECTED at S=1 (13% regression, 32-reg carry + issue-slot contention). Retested at S=2 the carry is 24 pairs/lane (48 regs, the fill splits across 64 threads) — AND the S>1 load/store phases had a bug: the single cooperative flat walk collapsed the per-subgroup B regions (every subgroup modulo-mapped the whole B space onto its own slice → misplaced stores, rel ~3e-2). FIXED: the load/store phases now mirror the fused fill's two-phase walk (A cooperative c_wg-stride, then B per-subgroup c32-stride past the A region). In-process A/B vs the fused 1-barrier (4096³, 5 rounds, both 4.436e-03 OK): clean rounds **0.891/0.921/0.914/0.922** → **~4.69-4.89ms vs 5.08-5.49ms**. Solo ~4.55ms = 30.2 TFLOP/s. | fused 1-barrier 5.08-5.49ms | **The S=1 rejection was register-bound, not structural — S=2 halves the per-lane carry. The D2 structure (full fill-latency hidden under the mma) is the portable cp.async emulation; it now works.** |
