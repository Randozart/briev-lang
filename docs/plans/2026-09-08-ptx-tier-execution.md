...
| Step | Gate |
|------|------|
| S1 | PTX saxpy harness: exact vs host reference on BOTH devices (3060 + 1070 Ti) |
| S2a | `--backend ptx` routes an existing GEMM `.abv`; y == SPIR-V path (rel 0.0) |
| S3 | single-mma / single-ldmatrix microtests with known fragments |
