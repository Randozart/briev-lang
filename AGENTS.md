...
| **Followup stages (2026-09-24, active umbrella)** | `docs/plans/2026-09-24-followup-stages.md` — the post-metaprogramming queue: Front D deferred-emitter retirement A/B → cross-dialect interop waves 1-3 → GPU re-rank (attention 198→125 µs, S3b GEMM, warp-slice thresholds, M4 ladder). Excludes live foreign branch `feat/e14a-intent-synthesis` |
...
| **Briev vs CUDA structural-capability thesis** | `docs/architecture/briev-vs-cuda-thesis.md` — why Briev's reactive-node-DAG + contracts architecture enables proof-based correctness, program-derived scheduling, shape-driven strategy synthesis, and reference-verified codegen — automatic behaviors CUDA's kernel-as-unit model has no mechanism to produce. The strongest claim: the contract-proved compiler writing itself |
